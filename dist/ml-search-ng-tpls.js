...
module.run(['$templateCache', function($templateCache) {
  $templateCache.put('/templates/ml-chiclets.html',
    '<div class="chiclets"><div ng-repeat="(index, facet) in activeFacets | object2Array"><div class="btn btn-primary chiclet" ng-repeat="value in facet.values | filter:{negated:false}"><span title="{{ value.value }}">{{ facet.__key }}: {{ value.value | truncate:truncateLength }}</span> <span class="fa fa-ban icon-white" ng-click="toggle({facet: facet.__key, value: value.value})"></span></div><div class="btn btn-warning chiclet negated" ng-repeat="value in facet.values | filter:{negated:true}"><span title="{{ value.value }}">{{ facet.__key }}: {{ value.value | truncate:truncateLength }}</span> <span class="fa fa-ban icon-white" ng-click="toggle({facet: facet.__key, value: value.value})"></span></div></div></div>');
}]);
})();
...
module.run(['$templateCache', function($templateCache) {
  $templateCache.put('/templates/ml-facets.html',
    '<div class="facet-list"><ml-chiclets ng-if="shouldNegate" active-facets="activeFacets" toggle="toggle({facet:facet, value:value})" truncate="{{ truncateLength }}"></ml-chiclets><div class="chiclets" ng-if="!shouldNegate"><div ng-repeat="(index, facet) in facets | object2Array | filter:{selected: true}"><div class="btn btn-primary" ng-repeat="value in facet.facetValues | filter:{selected: true}"><span title="{{ value.name }}">{{ facet.__key }}: {{ value.name | truncate:truncateLength }}</span> <span class="fa fa-ban icon-white" ng-click="toggle({facet: facet.__key, value: value.name})"></span></div></div></div><div class="facet" ng-if="filter(facet.facetValues, {selected: \'!\'+true}, false).length" ng-repeat="(index, facet) in facets | object2Array"><h3>{{ facet.__key }}</h3><div ng-repeat="value in facet.facetValues | filter:{selected: \'!\'+true}:false"><i class="fa fa-plus-circle facet-add-pos" ng-click="toggle({facet: facet.__key, value: value.name})"></i> <span ng-if="!!value.name" title="{{ value.name }}">{{ value.name | truncate:truncateLength }}</span> <em ng-if="!value.name">blank</em> <span>({{ value.count }})</span> <i class="fa fa-minus-circle facet-add-neg" ng-if="shouldNegate" ng-click="negate({facet: facet.__key, value: value.name})" title="{{ value.name }}"></i></div><div ng-if="shouldShowMore &amp;&amp; !facet.displayingAll"><a href="" ng-click="showMore({facet: facet, facetName: facet.__key})">see more ...</a></div></div></div>');
}]);
})();
...
module.run(['$templateCache', function($templateCache) {
  $templateCache.put('/templates/ml-results.html',
    '<div ng-repeat="result in results"><h4><a ng-if="shouldClick" ng-click="click({ result: result })">{{ result.label || result.uri }}</a> <a ng-if="!shouldClick" ng-href="{{ result.link }}">{{ result.label || result.uri }}</a></h4><div class="matches"><div class="match" ng-repeat="match in result.matches"><em ng-repeat="text in match[\'match-text\'] track by $index"><span ng-class="{ highlight: text.highlight !== undefined }"><span ng-if="text.highlight !== undefined">{{ text.highlight }}</span> <span ng-if="text.highlight === undefined">{{ text }}</span></span></em></div></div><hr></div>');
}]);
})();
